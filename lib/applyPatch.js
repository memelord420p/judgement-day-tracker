...
function applyOp(state, op) {
  ...
  switch (op.op) {
    ...
    case 'set_monitoring': {
      if (!state.monitoring) state.monitoring = {};
      if (op.watchQueries) state.monitoring.watchQueries = op.watchQueries;
      if (op.fighterWatch) state.monitoring.fighterWatch = { ...(state.monitoring.fighterWatch || {}), ...op.fighterWatch };
      if (op.gapHunt) state.monitoring.gapHunt = op.gapHunt;
      if (op.cadence) state.monitoring.cadence = { ...(state.monitoring.cadence || {}), ...op.cadence };
      log('updated monitoring');
      break;
    }
